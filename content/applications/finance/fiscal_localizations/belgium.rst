...
#. Go to :menuselection:`Accounting --> Configuration --> Accounting: CodaBox`.
#. Click on :guilabel:`Manage Connection` to open the connection wizard which will show the
   :guilabel:`Fiduciary VAT` number and the :guilabel:`Company VAT` number that will be used for
   the connection. If they are incorrect, they can be changed in
   :menuselection:`Settings --> Users & Companies --> Companies`.
#. Click on :guilabel:`Create connection`.

    * If this is your **first connection**, the wizard will confirm that the connection has been
      created on **Odoo's side**. Follow the steps to validate the connection on **CodaBox's side**
      too.

      .. image:: belgium/codabox_connection_validation.png
        :align: center
        :alt: Connection validation wizard

      .. note::
        This :guilabel:`Fiduciary Password` will be used to create new connections
        for other clients without having to validate the connection again. This password
        is unique to Odoo and must be stored securely on your side.

    * If this is **not your first connection**, the :guilabel:`Fiduciary password` provided by Odoo
      during the first connection will be requested to create a new connection.

      .. image:: belgium/codabox_connection_password.png
        :align: center
        :alt: Password request wizard
#. The :guilabel:`Status` should have now switched to :guilabel:`Connected`.

    .. image:: belgium/codabox_connection_ok.png
       :align: center
       :alt: Connection status OK
...
No connection exists with these fiduciary and company VAT numbers. Please check your configuration.
  This can happen when checking the connection status, and the pair (Fiduciary VAT, Company VAT)
  is not registered yet. This may happen if you have changed the company VAT after the connection
  was established. For security reasons, you have to recreate a connection for this company VAT.

It seems that your CodaBox connection is not valid anymore. Please connect again.
  This can happen if you have revoked the access of Odoo to your CodaBox account, or that you have
  not completed the configuration process. In this case, you must revoke your connection and create
  a new one.

It seems that you have already created an Odoo-CodaBox connection with this fiduciary. You must reuse the password you received from Odoo during your first connection.
  You must use the password you received from Odoo during your first connection to create a new
  connection for this fiduciary. If you have lost your password, you will have to first revoke the
  Odoo connection on CodaBox's side (i.e. on your myCodaBox portal). Then, you will be able to
  revoke the connection on Odoo's side and create a new one.

The provided password is not valid for this fiduciary.
    The password you provided is not the one you received from Odoo during your first connection.
    You must use the password you received from Odoo during your first connection to create a new
    connection for this fiduciary. If you have lost your password, you will have to first revoke the
    Odoo connection on CodaBox's side (i.e. on your myCodaBox portal). Then, you will be able to
    revoke the connection on Odoo's side and create a new one.

It seems that the company or fiduciary VAT number you provided is not valid. Please check your configuration.
...
It seems that the fiduciary VAT number you provided does not exist in CodaBox. Please check your configuration.
  The Fiduciary VAT number you provided is not registered in CodaBox. You may not have a valid
